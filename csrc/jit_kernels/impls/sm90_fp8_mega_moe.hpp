...
// SM90 (Hopper) FP8 MegaMoE host runtime, driving the N-split kernel in
// `deep_gemm/impls/sm90_fp8_mega_moe.cuh`. Unlike SM100: FP8-only (no FP4),
// per-128 channel float SF, register-resident WGMMA accumulators, cluster=1.
...
static void sm90_fp8_mega_moe(
    ...
    // Activation SF: per-128 channel float for both L1 and L2 (MN-major, no swizzle).
    ...
    // L1 output (post-SwiGLU FP8): one L1 block emits `block_n / 2` = 128 columns for
    // all `block_m` rows. The SMEM staging tile is plain row-major, so the store
    // descriptor uses no swizzle; the outer-box dim is the full `block_m` (both WGs
    // write disjoint column halves of one tile, then a single full-tile TMA store).
