...
// ============================================================================
// SM90 (Hopper) FP8 MegaMoE host runtime — pingpong kernel
// ----------------------------------------------------------------------------
// SM90 counterpart of `SM100FP8FP4MegaMoERuntime`, driving the pingpong kernel
// in `deep_gemm/impls/sm90_fp8_mega_moe_pingpong.cuh` (BLOCK_M=64, one math
// warpgroup per tile, the two warpgroups overlapping one's MMA with the other's
// epilogue). Best for small/medium M.
//
...
class SM90FP8MegaMoEPingpongRuntime final : public LaunchRuntime<SM90FP8MegaMoEPingpongRuntime> {
public:
    struct Args {
        // Templated arguments
...
#include <deep_gemm/impls/sm90_fp8_mega_moe_pingpong.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm90_fp8_mega_moe_pingpong_impl<
        {},
        {}, {},
        {}, {},
        {},
...
static void sm90_fp8_mega_moe_pingpong(
    ...
    // Pingpong: each math warpgroup owns the WHOLE BLOCK_M tile and stores it in
    // a single TMA op, so the descriptor outer-box dim must be `block_m` (the
    // full tile), NOT `block_m / num_epilogue_warpgroups` — otherwise rows
    // beyond the half-tile are silently dropped by the TMA store.
    ...
    const SM90FP8MegaMoEPingpongRuntime::Args args = {
    ...
    const auto code = SM90FP8MegaMoEPingpongRuntime::generate(args);
    const auto runtime = compiler->build("sm90_fp8_mega_moe_pingpong", code);
    SM90FP8MegaMoEPingpongRuntime::launch(runtime, args);
}
