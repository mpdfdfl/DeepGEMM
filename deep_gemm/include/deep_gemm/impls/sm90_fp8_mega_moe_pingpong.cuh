...
// ============================================================================
// SM90 (Hopper) FP8 MegaMoE — pingpong kernel
// ----------------------------------------------------------------------------
// BLOCK_M=64: each math warpgroup owns a whole tile and the two warpgroups run
// half a cycle out of phase (one does WGMMA while the other runs its epilogue),
// handing off via an OrderedSequenceBarrier. Tuned for small/medium M.
//
...
sm90_fp8_mega_moe_pingpong_impl(void* y,
...
#pragma unroll
        ...
        if (cute::elect_one_sync()) {
            // Pingpong: each pipeline stage is consumed by exactly ONE math WG,
            // so its `empty` barrier is released by that WG's warps only.
...
#pragma unroll
            for (uint32_t i = 0; i < kNumStages; ++i) {
                ...
                // One arrive per warp of the owning WG (lane 0 of each warp).
                empty_barriers[i]->init(kNumWarpsPerEpilogueWG);
            }
#pragma unroll
            for (uint32_t i = 0; i < kNumEpilogueWarps * 2; ++i)
                combine_barriers[i]->init(1);
            // OrderedSequenceBarrier<2,2>: all 128 threads of the signalling WG arrive.
#pragma unroll
...
#pragma unroll
        ...
        // Pingpong: each math WG owns a WHOLE BLOCK_M tile (one m64 WGMMA); the
        // two WGs alternate tiles (WG g owns stream positions where pos%2==g).
        constexpr uint32_t WG_BLOCK_M = BLOCK_M;
        DG_STATIC_ASSERT(BLOCK_M == L1WGMMA::M, "Pingpong: BLOCK_M must equal one WGMMA M (64)");
        DG_STATIC_ASSERT(kNumEpilogueWarpgroups == 2, "Pingpong requires exactly 2 math warpgroups");

        // Sync with dispatch
        ptx::sync_unaligned(kNumDispatchThreads + kNumEpilogueThreads, kDispatchWithEpilogueBarrierIdx);

        // Pingpong order barrier (OrderedSequenceBarrier<2,2>): WG0=leader starts
        // phase 1 (on a fresh barrier `wait(1)` passes immediately, `wait(0)`
        // blocks — same convention as the loader's `empty.wait(phase^1)`), so
        // WG0's first MMA wait passes and WG1=follower (phase 0) blocks until
        // WG0 arrives. `order_arrive` signals the other WG and steps the local
        // state machine; stage 0 gates the MMA region, stage 1 gates the epilogue.
        uint32_t ord_stage = 0;
        uint32_t ord_phase = (epilogue_wg_idx == 0) ? 1u : 0u;
        const auto order_wait = [&]() {
            order_barriers[ord_stage * 2 + epilogue_wg_idx]->wait(ord_phase);
        };
        const auto order_arrive = [&]() {
            order_barriers[ord_stage * 2 + (epilogue_wg_idx ^ 1u)]->arrive();
            ord_stage ^= 1u;
            if (ord_stage == 0)
                ord_phase ^= 1u;
        };
        ...
        while (true) {
            ...
            // Blocks not owned by this WG: only advance the shared pipeline cursor
            // (stage/phase) so the next owned block lands on the correct ring slot.
            // The owning WG drives the full/empty barriers for this block.
            if (pos % 2 != epilogue_wg_idx) {
                for (uint32_t k = 0; k < num_k_blocks; advance_pipeline(k)) {}
                ++pos;
                continue;
            }
            ...
            // ---------------- GEMM (MMA region: serialized via order stage 0) ----------------
            order_wait();
            using WGMMA                        = L1WGMMA;
...
#pragma unroll
                    ...
                    scale_a_0_lo = ptx::ld_shared(smem_sfa[stage_idx] + r_0);
                    scale_a_1_lo = ptx::ld_shared(smem_sfa[stage_idx] + r_1);
...
#pragma unroll
                    ...
                    scale_a_0_lo = ptx::ld_shared(smem_sfa[stage_idx] + 0 * BLOCK_M + r_0);
                    scale_a_1_lo = ptx::ld_shared(smem_sfa[stage_idx] + 0 * BLOCK_M + r_1);
...
#pragma unroll
                    ...
                    scale_a_0_hi = ptx::ld_shared(smem_sfa[stage_idx] + 1 * BLOCK_M + r_0);
                    scale_a_1_hi = ptx::ld_shared(smem_sfa[stage_idx] + 1 * BLOCK_M + r_1);
...
#pragma unroll
            ...
            // Hand the MMA region (tensor cores) to the other WG, then wait our
            // turn for the epilogue region — this is the pingpong overlap point:
            // the other WG runs its MMA while this WG runs its epilogue.
            order_arrive();
            order_wait();
...
#pragma unroll
                ...
                float weight_r0 = *l1_topk_weights_buffer
                                       .get_data_buffer(m_idx + r_0)
                                       .get_base_ptr<float>();
                float weight_r1 = *l1_topk_weights_buffer
                                       .get_data_buffer(m_idx + r_1)
                                       .get_base_ptr<float>();
#pragma unroll
                for (uint32_t p = 0; p < kNumPairs; ++p) {
                ...
                // Both WGs reuse the same smem_cd staging tile; the order barrier
                // (stage 1) guarantees only one WG is in the epilogue at a time.
...
#pragma unroll
                ...
                // Pingpong: this WG owns the whole tile, so its own warp 0 signals.
                ptx::tma_store_wait<0>();
                ptx::sync_aligned(128, kEpilogueWGBarrierStartIdx + epilogue_wg_idx);
                if (warp_idx_in_wg == 0 and cute::elect_one_sync()) {
                    ptx::red_or_rel_gpu(
                ...
                __syncwarp();
                // Hand the epilogue region to the other WG (stage 1).
                order_arrive();
            } else {
...
#pragma unroll
                for (uint32_t i = 0; i < kAccumPerThread / 8; ++i) {
                    ...
                    auto write_pair = [&](uint32_t row, uint32_t col, uint32_t packed) {
                        auto smem_ptr = smem_cd_l2 + row * BLOCK_N + col;
...
#pragma unroll
                for (uint32_t j = 0; j < kNumRowsPerWarp; ++j) {
                    ...
                    if (row_in_wg >= valid_m)
                        break;

                    const auto src_metadata      = *workspace.get_token_src_metadata_ptr(m_idx + row_in_wg);
                    ...
                    auto smem_ptr     = smem_cd_l2 + row_in_wg * BLOCK_N + lane_in_row * cols_per_lane;
                ...
                // WG-local sync, then hand the epilogue region to the other WG.
                ptx::sync_aligned(128, kEpilogueWGBarrierStartIdx + epilogue_wg_idx);
                order_arrive();
            }
            ++pos;
